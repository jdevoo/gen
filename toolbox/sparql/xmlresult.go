...
func (*XMLResultParser) Parse(ctx context.Context, r io.ReadCloser) (QueryResult, error) {
	return DecodeXMLQueryResult(ctx, r)
}
...
func DecodeXMLQueryResult(ctx context.Context, r io.ReadCloser) (QueryResult, error) {
...
// Next retrieves the next result set. The context is checked before each read.
func (x *XMLQueryResult) Next() (map[string]Value, error) {
	if len(x.variables) == 0 {
		return nil, fmt.Errorf("sparql: Next() called on an ASK query result")
	}
	for {
		select {
		case <-x.ctx.Done():
		...
		default:
			// Proceed with decoding only if not cancelled
			t, err := startElement(x.decoder)
			if err != nil {
				if err == io.EOF {
					return map[string]Value{}, io.EOF
				}
				return nil, err
			}
...
// decodeResult returns the bindings as a map.
...
func (x *XMLQueryResult) Boolean() (bool, error) {
	if len(x.variables) > 0 {
		return false, fmt.Errorf("sparql: Boolean() called on a SELECT query result")
	}
	for {
		select {
		case <-x.ctx.Done():
			return false, x.ctx.Err()
		default:
			t, err := startElement(x.decoder)
			if err != nil {
				if err == io.EOF {
